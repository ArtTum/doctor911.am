...
class Helper extends Component{
     ...
     * @param bool $type
     */
    public static function out($data = false, $type = false)
    {
        echo '<pre>';
        if(!$type)
            print_r($data);
     ...
     */
    public static function ratingDoctor($doctor_id){

        $comment = Comment::find()
            ->where(['status'=>1, 'doctor_id' => $doctor_id])
            ->andWhere(['not', ['star' => null]]);

        return ($comment->sum('star') ? $comment->sum('star')  / $comment->count() : null);
     ...
     * @return int|string
     */
    public static function ratingDoctorCount($doctor_id){
     ...
     * @return float|int|null
     */
    public static function ratingHospital($hospital_id){
     ...
     * @return int|string
     */
    public static function ratingHospitalCount($hospital_id){
    ...
    public static function languageUrl(){
        ...
        $languages = \common\models\Lang::find()->all();
     ...
     * @return string
     */
    public static function deyElapsed($fromTime = null, $human = true, $toTime = null, $append = null)
    {
        static::initI18N();
        ...
        return static::timeInterval($toTime - $fromTime, $human);
    }
