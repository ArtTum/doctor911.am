...
/**
 * This is the model class for table "lang_has_news".
 *
 ...
 * @property News $news
 */
class LangHasNews extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return [
            [['lang_id', 'news_id', 'name', 'description'], 'required'],
            [['lang_id', 'news_id'], 'integer'],
            [['description', 'meta_description'], 'string'],
            [['name', 'meta_title', 'meta_keys'], 'string', 'max' => 255],
            [['lang_id', 'news_id'], 'unique', 'targetAttribute' => ['lang_id', 'news_id']],
    ...
    public function add($post, $lang, $model_id){
        ...
        foreach($lang as $item){
            $model = new LangHasNews();
            $model->lang_id = $item->id;
            $model->news_id = $model_id;
    ...
    public function remove($model_id){
        LangHasNews::deleteAll(['news_id' => $model_id]);
    }
}
