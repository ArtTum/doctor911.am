...
/**
 * This is the model class for table "lang_has_page".
 *
 ...
 * @property Page $page
 */
class LangHasPage extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return [
            [['lang_id', 'page_id', 'name', 'description'], 'required'],
            [['lang_id', 'page_id'], 'integer'],
            [['description', 'meta_description'], 'string'],
            [['name', 'meta_title', 'meta_keys'], 'string', 'max' => 255],
            [['lang_id', 'page_id'], 'unique', 'targetAttribute' => ['lang_id', 'page_id']],
    ...
    public function add($post, $lang, $model_id){
        ...
        foreach($lang as $item){
            $model = new LangHasPage();
            $model->lang_id = $item->id;
            $model->page_id = $model_id;
    ...
    public function remove($model_id){
        LangHasPage::deleteAll(['page_id' => $model_id]);
    }
}
